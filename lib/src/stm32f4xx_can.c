...
uint8_t CAN_Init(CAN_TypeDef* CANx, CAN_InitTypeDef* CAN_InitStruct)
{
    ...
    if ((CANx->MSR & CAN_MSR_INAK) != CAN_MSR_INAK) {
        ...
        CANx->BTR =
            (uint32_t)((uint32_t)CAN_InitStruct->CAN_Mode << 30) | ((uint32_t)CAN_InitStruct
                ->CAN_SJW << 24) | ((uint32_t)CAN_InitStruct->CAN_BS1 << 16) | ((uint32_t)CAN_InitStruct
...
void CAN_FilterInit(CAN_FilterInitTypeDef* CAN_FilterInitStruct)
{
    ...
    filter_number_bit_pos = ((uint32_t)1) << CAN_FilterInitStruct
        ->CAN_FilterNumber;
    ...
    if (CAN_FilterInitStruct->CAN_FilterScale == CAN_FilterScale_16bit) {
        ...
        CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR1 =
            ((0x0000FFFF & (uint32_t)CAN_FilterInitStruct->CAN_FilterMaskIdLow) << 16) | (0x0000FFFF & (uint32_t)CAN_FilterInitStruct
                ->CAN_FilterIdLow);
        
        /* Second 16-bit identifier and Second 16-bit mask */
        /* Or Third 16-bit identifier and Fourth 16-bit identifier */
        CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR2 =
            ((0x0000FFFF & (uint32_t)CAN_FilterInitStruct->CAN_FilterMaskIdHigh) << 16) | (0x0000FFFF & (uint32_t)CAN_FilterInitStruct
                ->CAN_FilterIdHigh);
    }
    ...
    if (CAN_FilterInitStruct->CAN_FilterScale == CAN_FilterScale_32bit) {
        ...
        CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR1 =
            ((0x0000FFFF & (uint32_t)CAN_FilterInitStruct->CAN_FilterIdHigh) << 16) | (0x0000FFFF & (uint32_t)CAN_FilterInitStruct
                ->CAN_FilterIdLow);
...
uint8_t CAN_Transmit(CAN_TypeDef* CANx, CanTxMsg* TxMessage)
{
    ...
    if (transmit_mailbox != CAN_TxStatus_NoMailBox) {
        ...
        CANx->sTxMailBox[transmit_mailbox].TDLR =
            (((uint32_t)TxMessage->Data[3] << 24) | ((uint32_t)TxMessage->Data[2] << 16) | ((uint32_t)TxMessage
                ->Data[1] << 8) | ((uint32_t)TxMessage->Data[0]));
        CANx->sTxMailBox[transmit_mailbox].TDHR =
            (((uint32_t)TxMessage->Data[7] << 24) | ((uint32_t)TxMessage->Data[6] << 16) | ((uint32_t)TxMessage
                ->Data[5] << 8) | ((uint32_t)TxMessage->Data[4]));
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef* CANx, uint8_t CAN_OperatingMode)
{
    ...
    if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
        /* Request initialisation */
        CANx->MCR =
            (uint32_t)((CANx->MCR & (uint32_t)(~(uint32_t)CAN_MCR_SLEEP)) | CAN_MCR_INRQ);
        ...
        /* Request Sleep mode */
        CANx->MCR =
            (uint32_t)((CANx->MCR & (uint32_t)(~(uint32_t)CAN_MCR_INRQ)) | CAN_MCR_SLEEP);
